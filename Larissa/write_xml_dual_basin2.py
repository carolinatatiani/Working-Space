...
# Last Modified: 07/07/2025# Author: Carolina Tatiani
...
def write_cosine_squared(root, interactions, name="cossquare", w=1):
    dihedrals = root.find("dihedrals")
    if dihedrals is None:
        dihedrals = ET.SubElement(root, "dihedrals")
    
    dihedrals_type = ET.SubElement(dihedrals, "dihedrals_type", name=name)
    expression = ET.SubElement(dihedrals_type, "expression")
    expression.set("expr", "A*(1-(cos(mult*(phi-phi0))^2)")
    parameters = ["A", "phi0", "mult"]
    for parameter_name in parameters:
        parameter = ET.SubElement(dihedrals_type, "parameter")
        parameter.text = parameter_name

    for interaction in interactions:
        interaction_element = ET.SubElement(dihedrals_type, "interaction")
        interaction_element.set("i", str(interaction[0]))
        interaction_element.set("j", str(interaction[1]))
        interaction_element.set("k", str(interaction[2]))
        interaction_element.set("l", str(interaction[3]))
        interaction_element.set("func", str(interaction[4]))
        interaction_element.set("phi0", str(interaction[5]))
        interaction_element.set("kd", str(interaction[6]))
        interaction_element.set("mult", str(interaction[7]))
